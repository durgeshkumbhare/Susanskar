...
const Hero = () => {
  ...
  return (
    <div className="overflow-hidden relative h-[32rem]  w-[100%] ">
      {images.map((image, imageIndex) => (
        ...
        >
          <img
            src={image.img}
            className="h-[30rem] w-full  object-cover border-2 border-black"
          />
        </div>
      ))}
      <div className="">
        <IoIosArrowBack
          onClick={prevSlide}
          className="text-3xl absolute  mt-60 left-4  text-white cursor-pointer hover:scale-105 hover:text-gray-600"
        />
        <IoIosArrowForward
          onClick={nextSlide}
          className="text-3xl absolute  mt-60 right-4  text-white cursor-pointer hover:scale-105 hover:text-gray-600"
        />
      </div>
    </div>
  );
